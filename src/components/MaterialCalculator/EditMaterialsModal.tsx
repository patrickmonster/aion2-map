...
const EditMaterialsModal: React.FC<EditMaterialsModalProps> = ({
  shopData,
  getItemById,
  itemId,
  currentItem,
  ...
  useEffect(() => {
    if (
      ...
      currentItem.ingredient.length > 0
    ) {
  ...
  const handleAddMaterial = (shopItem: ShopItem) => {
    const existingMaterial = selectedMaterials.find(
      (m) => m.itemId === shopItem.id
    );
    if (existingMaterial) {
      // 이미 있으면 수량 증가
      setSelectedMaterials((prev) =>
        prev.map((m) =>
          m.itemId === shopItem.id ? { ...m, quantity: m.quantity + 1 } : m
        )
      );
    } else {
      // 새로 추가
  ...
  const handleQuantityChange = (itemId: string, quantity: number) => {
    ...
    setSelectedMaterials((prev) =>
      prev.map((m) => (m.itemId === itemId ? { ...m, quantity } : m))
    );
  };
