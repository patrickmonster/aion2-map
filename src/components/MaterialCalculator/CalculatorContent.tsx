...
// 정렬 타입 정의
type SortField = 'name' | 'type' | 'minPrice' | 'maxPrice' | 'minProfit' | 'maxProfit';
...
const CalculatorContent: React.FC<CalculatorContentProps> = ({ className }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const customItems: CalculatorItem[] = localCalculatorData.map(item => {
        ...
        const materialAverageCost = Math.round((materialMinCost + materialMaxCost) / 2);
    ...
    loadData();
  }, [activeMainTab, getTabCategory, loadCalculatorData, loadItemOverrides, getItemById]);
  ...
  // 재료 편집 모달 상태
  const [isEditMaterialsModalOpen, setIsEditMaterialsModalOpen] = useState(false);
  ...
  const handleDeleteItem = useCallback(
    (itemId: string) => {
      if (window.confirm('이 아이템을 삭제하시겠습니까?')) {
        ...
        setCustomCalculatorItems(prev => prev.filter(item => item.id !== itemId));
      }
    },
  ...
  // 상점 아이템 저장 핸들러
  const handleSaveShopItem = useCallback((itemId: string, updatedItem: Partial<ShopItem>) => {
    setShopData(prev => prev.map(item => (item.id === itemId ? { ...item, ...updatedItem } : item)));
  }, []);
  ...
  const handleSyncData = useCallback(async () => {
    ...
    if (result.syncedCount > 0) {
      ...
      const customItems: CalculatorItem[] = updatedData.map(item => ({
        ...
        minProfit: 0,
        maxProfit: 0,
        materials: item.materials?.length > 0 ? `${item.materials.length}개 재료` : '재료 없음',
        materialCount: item.materials?.length || 0,
        ingredient: item.materials || [],
      }));
      setCustomCalculatorItems(customItems);
  ...
  }, [activeMainTab, getTabCategory, getRecipeFileName, syncCalculatorData, getItemById, loadCalculatorData]);

  // 로컬 데이터 삭제 및 동기화 핸들러
  const handleClearAndSync = useCallback(async () => {
    if (!window.confirm('로컬 데이터를 모두 삭제하고 다시 동기화하시겠습니까?')) {
      return;
    }
  ...
  const handleAddCalculatorItem = useCallback(
    (selectedShopItems: ShopItem[]) => {
      ...
      const newCalculatorItems: CalculatorItem[] = selectedShopItems.map(selectedShopItem => ({
      ...
      // 로컬 스토리지에서 현재 데이터 로드
      const currentData = loadCalculatorData(category);

      // 새로운 항목을 CalculatorItemData 형식으로 변환
      const newDataItems: CalculatorItemData[] = newCalculatorItems.map(item => ({
        id: item.id,
        name: item.name,
        type: item.type,
        combo: item.combo,
        cnt: item.cnt,
        materials: item.ingredient,
        lastUpdated: item.lastUpdated,
        hasUpperGradeCombination: item.hasUpperGradeCombination,
      }));

      // 중복 제거: 이미 존재하는 ID는 추가하지 않음
      const existingIds = new Set(currentData.map(item => item.id));
      const itemsToAdd = newDataItems.filter(item => !existingIds.has(item.id));

      if (itemsToAdd.length > 0) {
        // 로컬 스토리지에 저장
        const updatedData = [...currentData, ...itemsToAdd];
        updateCalculatorItem(category, '', { materials: [] }); // 더미 호출로 저장 트리거

        // 직접 저장
        localStorage.setItem(`aion2-calculate-${category}`, JSON.stringify(updatedData));
    ...
    [activeMainTab, getTabCategory, loadCalculatorData, updateCalculatorItem]
  );
  ...
  const calculateMaterialCosts = useCallback(
    (materials: { itemId: string; quantity: number }[], excludeConvertFlag: boolean) => {
      let minCost = 0;
      let maxCost = 0;
  ...
  const filteredItems = useMemo(() => {
    ...
    return items;
  }, [allCalculatorItems, searchTerm, materialFilter, typeFilter, sortState, itemOverrides, excludeConvert, calculateMaterialCosts, getItemById]);
  ...
  return (
    <>
      ...
      <div className="calculator-header">
        <button onClick={() => setIsAddModalOpen(true)} className="btn-add-item">
          + 계산 항목 추가
        </button>
        <button onClick={handleSyncData} className="btn-sync-data" disabled={isCalculatorLoading}>
          {isCalculatorLoading ? '동기화 중...' : '🔄 데이터 동기화'}
        </button>
        <button onClick={handleClearAndSync} className="btn-clear-data" disabled={isCalculatorLoading}>
      ...
      <div className="search-filters">
        <div className="search-group">
          ...
          <input id="search-name" type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="상품명으로 검색..." className="search-input" />
        </div>
        <div className="search-group">
          <label htmlFor="search-material">재료:</label>
          <input id="search-material" type="text" value={materialFilter} onChange={e => setMaterialFilter(e.target.value)} placeholder="재료명으로 검색..." className="search-input" />
        </div>
        <div className="search-group">
          <TypeSelector value={typeFilter} onChange={setTypeFilter} label="타입:" className="search-select" />
        </div>
        <div className="search-group">
          ...
          <table className="calculator-table">
            <thead>
              <tr>
                ...
                <th className="sortable" onClick={() => handleSort('type')} style={{ cursor: 'pointer' }}>
                  타입{getSortIcon('type')}
                </th>
                <th className="sortable" onClick={() => handleSort('minPrice')} style={{ cursor: 'pointer' }}>
                  최저가{getSortIcon('minPrice')}
                </th>
                <th className="sortable" onClick={() => handleSort('maxPrice')} style={{ cursor: 'pointer' }}>
                  최고가{getSortIcon('maxPrice')}
                </th>
                <th>제작비용</th>
                <th className="sortable" onClick={() => handleSort('minProfit')} style={{ cursor: 'pointer' }}>
                  최저이윤{getSortIcon('minProfit')}
                </th>
                <th className="sortable" onClick={() => handleSort('maxProfit')} style={{ cursor: 'pointer' }}>
              ...
              {filteredItems.length > 0 ? (
                filteredItems.map(item => (
                  <tr key={`calculate-${item.id}`}>
                    ...
                    <td className="materials-cell clickable" onClick={() => handleMaterialsClick(item.id)} title="클릭하여 재료 편집">
                      {item.materials}
                    </td>
                    ...
                    <td className="action-buttons">
                      ...
                      <button className="btn-delete" onClick={() => handleDeleteItem(item.id)} title="아이템 삭제">
                        삭제
                      </button>
                    </td>
                  </tr>
      ...
      {/* 계산 항목 추가 모달 */}
      {isAddModalOpen && <AddCalculatorItemModal shopData={shopData} onClose={() => setIsAddModalOpen(false)} onSelect={handleAddCalculatorItem} />}
            ...
            setCustomCalculatorItems(prev =>
              prev.map(item => {
                if (item.id === itemId) {
                  ...
                  const materialsText = materialNames.length === 1 ? materialNames[0] : `${materialNames[0]} 외 ${materialNames.length - 1}개`;
