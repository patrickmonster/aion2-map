...
const Map: React.FC = () => {
  ...
  const handleSaveMemo = useCallback(() => {
    ...
    uiState.memoText,
    selectedMap,
    uiState.editingMarker,
    uiState.selectedMarkerType,
    saveMarker,
  ]);
    ...
    <div className="map-container">
      ...
      <MemoDialog
        ...
        onMarkerTypeChange={(markerType) =>
          uiDispatch({ type: "SET_SELECTED_MARKER_TYPE", markerType })
        }
      ...
      <MarkersSidebar
        ...
        onFilterChange={(filter) =>
          uiDispatch({ type: "SET_MARKER_FILTER", filter })
        }
      ...
      {/* 마커 타입 추가/편집 폼 */}
      {uiState.showMarkerTypeForm && (
        <MarkerTypeForm
          markerType={uiState.editingMarkerType}
          onSave={handleSaveMarkerType}
          onCancel={() => uiDispatch({ type: "CLOSE_MARKER_TYPE_FORM" })}
        />
      )}
      ...
      {uiState.isAdminMode && (
        <JsonEditorModal
          isOpen={uiState.showJsonEditor}
          onClose={() => uiDispatch({ type: "CLOSE_JSON_EDITOR" })}
          selectedMap={selectedMap}
          gameData={gameData}
        />
      )}
